...
class GPT3_Reasoning_Graph_Baseline:
    ...
    def load_inference(self):
        file_path = os.path.join('./utils/prompt','inference.txt')
        with open(file_path) as f:
            in_context_examples = f.read()
        return in_context_examples
    ...
    def batch_reasoning_graph_generation(self, batch_size=10):
        ...
        for chunk in tqdm(dataset_chunks):
            try:
                ...
                print("Reasoning...")
                # import pdb;pdb.set_trace()
                batch_reasoning = self.openai_api.batch_generate([self.construct_prompt_CoT(example, in_context_examples_CoT) for example in chunk])

                batch_inference = self.openai_api.batch_generate([self.construct_prompt_inference(example,response, in_context_examples_infer) for example,response in zip(chunk,batch_reasoning)])
                
                end_time = time.time()
                time_cost = end_time - start_time
                for sample, reasoning,answer in zip(chunk,batch_reasoning,batch_inference):
                    # import pdb;pdb.set_trace()
               
                    dict_output = self.update_answer(sample,reasoning,answer, time_cost)
                ...
                for sample in chunk:
                    try:
                        ...
                        print("Reasoning...")
                        prompts_reasoning = self.construct_prompt_CoT(sample, in_context_examples_CoT)
                        reasoning, _ = self.openai_api.generate(prompts_reasoning)

                        prompts_inference = self.construct_prompt_inference(sample, reasoning,in_context_examples_infer)
                        answer, _ = self.openai_api.generate(prompts_inference)
    ...
    def update_answer(self,sample, reasoning,answer, time_cost):
        # import pdb;pdb.set_trace()
