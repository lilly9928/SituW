...
def fact_key(s: str, p: str, o: Optional[str]) -> str:
    return json.dumps(
        {"s": norm_entity(s) or "", "p": norm_predicate(p) or "", "o": norm_object(o) or ""},
        sort_keys=True
    )
...
# -------------------------
# (Deprecated) Rule-based parsing helpers
# -------------------------
# 룰 기반 파싱은 더 이상 사용하지 않지만, 기존 코드 호환을 위해 남겨둡니다.
...
def relevant_constants_from_situation_model(
...
class Stage2Reasoner:
    def __init__(self, args):
        ...
        # 룰 기반 conclusion 파싱 비활성화: conclusion 파싱은 LLM만 사용
    ...
    def parse_conclusions_llm(self, conclusions: List[str]) -> List[Optional[List[List[Dict[str, Any]]]]]:
        ...
        # 1) parse batch (실패 시 단건 fallback)
        try:
            outs = self._batch_generate(prompts)
            if (not isinstance(outs, list)) or (len(outs) != len(prompts)):
                raise RuntimeError(f"batch_generate returned {type(outs)} len={len(outs) if isinstance(outs, list) else 'NA'}")
        except Exception:
            outs = []
            for pr in prompts:
                try:
                    outs.append(self._single_generate(pr))
                except Exception:
                    outs.append("")  # 파싱 실패 처리로 흘러가게

        parsed = [first_json(o) for o in outs]

        # 2) validate batch (실패 시 단건 fallback)
        val_prompts = []
        ...
        try:
            ...
            if (not isinstance(val_outs, list)) or (len(val_outs) != len(val_prompts)):
                raise RuntimeError(f"batch_generate(validate) returned {type(val_outs)} len={len(val_outs) if isinstance(val_outs, list) else 'NA'}")
        except Exception:
            val_outs = []
            for pr in val_prompts:
                try:
                    val_outs.append(self._single_generate(pr))
                except Exception:
                    val_outs.append("")

        val_parsed = [first_json(o) for o in val_outs]

        # (선택) batch는 성공했지만 일부만 빈/파싱실패인 경우만 단건 재시도
        # - 비용 증가를 원치 않으면 이 블록은 지워도 됨.
        for i, j in enumerate(val_parsed):
            if j is None:  # validate 결과 JSON 파싱 실패
                try:
                    retry_out = self._single_generate(val_prompts[i])
                    val_parsed[i] = first_json(retry_out)
                except Exception:
                    pass
    ...
    def run(self):
        ...
        for fpath in stage1_files:
            ...
            # =========================
            # 결론 파싱: 룰 기반 제거 → LLM ONLY
            # =========================
            if not self.use_llm_conclusion:
                raise RuntimeError(
                    "Rule-based conclusion parsing is disabled. "
                    "Enable LLM parsing by setting --use_llm_conclusion "
                    "and provide --api_key/--model_name as needed."
                )

            parsed_queries: List[Optional[List[List[Dict[str, Any]]]]] = [None] * len(pids)
            llm_qs: List[Optional[List[List[Dict[str, Any]]]]] = []
            for j in range(0, len(conclusions), self.batch_size):
                llm_qs.extend(self.parse_conclusions_llm(conclusions[j:j + self.batch_size]))

            if len(llm_qs) != len(conclusions):
                raise RuntimeError(f"LLM parse count mismatch: {len(llm_qs)} vs {len(conclusions)}")

            for i, qq in enumerate(llm_qs):
                parsed_queries[i] = normalize_query_alternatives(qq) if qq else None
...
def parse_args():
    ...
    # 룰 기반 파싱 제거 → LLM 파싱이 기본 True
    p.add_argument("--use_llm_conclusion", action="store_true", default=True)
    p.add_argument("--no_use_llm_conclusion", dest="use_llm_conclusion", action="store_false")
